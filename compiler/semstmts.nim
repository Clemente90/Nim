...
proc semBorrow(c: PContext, n: PNode, s: PSym) =
  ...
  of bsMatch:
    ...
    let magicSym =
      if b.magic == mNone and b.instantiatedFrom != nil:
        b.instantiatedFrom
      else:
        b
    s.magic = magicSym.magic
    ...
    if s.name.s in ["[]", "[]="]:
      ...
      if paramType != nil:
        paramType = paramType.skipTypes({tyVar, tyLent, tyPtr, tyRef, tyOwned, tyAlias, tySink})
        case paramType.kind
        of tyDistinct:
          incl(paramType, tfBorrowBrackets)
        of tyGenericInvocation, tyGenericInst:
          let head = paramType.genericHead
          if head.last.kind == tyDistinct:
            incl(head.last, tfBorrowBrackets)
        else:
          discard
  of bsNoDistinct:
